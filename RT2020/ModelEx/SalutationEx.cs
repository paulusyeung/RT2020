    ...
    public class SalutationEx
    {
        public static Guid GetIdByCode(string code)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                ...
                if (item != null) result = item.SalutationId;
            }

            return result;
        }

        public static bool IsSalutationCodeInUse(string code)
        {
            bool result = false;
        ...
        /// <summary>
        /// Get a list of Salutation objects from the database
        /// </summary>
        /// <returns>A list containing all of the Salutation objects in the database.</returns>
        public static List<EF6.Salutation> GetList()
        {
            var whereClause = "1 = 1";
        ...
        /// </summary>
        /// <returns>A list containing all of the Salutation objects in the database ordered by the columns specified.</returns>
        public static List<EF6.Salutation> GetList(string whereClause)
        {
            var orderBy = new string[] { "SalutationId" };
        ...
        /// </summary>
        /// <returns>A list containing all of the Salutation objects in the database.</returns>
        public static List<EF6.Salutation> GetList(string whereClause, string[] orderBy)
        {
            List<EF6.Salutation> result = new List<EF6.Salutation>();
            ...
            {
                result = ctx.Salutation
                    .SqlQuery(string.Format("Select * from Salutation Where {0} Order By {1}", whereClause, orderby))
                    .AsNoTracking()
                    .ToList();
            }
        ...
        /// <summary>
        /// Deletes a EF6.Salutation object from the database.
        /// </summary>
        /// <param name="salutationId">The primary key value</param>
        public static bool Delete(Guid salutationId)
        {
            bool result = false;

            using (var ctx = new EF6.RT2020Entities())
            {
                var item = ctx.Salutation.Find(salutationId);
                if (item != null)
                {
		...
		public static void LoadCombo(ref ComboBox ddList, string TextField, bool SwitchLocale, bool BlankLine, string BlankLineText, string ParentFilter, string WhereClause, string[] OrderBy)
        {
            ...
            if (SwitchLocale)
            {
                ...
                {
                    if (OrderBy[i] == "SalutationName")
                    {
                        OrderBy[i] = CookieHelper.CurrentLocaleId == LanguageHelper.AlternateLanguage2.Key ?
                            "SalutationName_Cht" : CookieHelper.CurrentLocaleId == LanguageHelper.AlternateLanguage1.Key ?
                            "SalutationName_Chs" :
                            "SalutationName";
                    }
                }
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                ...
                var kvpList = ctx.Database.SqlQuery<ComboBoxHelper.ComboBoxItem>(
                    string.Format(
                        "Select SalutationId as [Key], {0} as [Value] from Salutation Where {1} Order By {2}",
		...
		public static void LoadCombo(ref ComboBox ddList, string[] TextField, string TextFormatString, bool SwitchLocale, bool BlankLine, string BlankLineText, string WhereClause, string[] OrderBy)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var list = ctx.Salutation.SqlQuery(
                    String.Format(
                        "Select * from Salutation Where {0} Order By {1}",
                ...
                foreach (var item in list)
                {
                    ...
                    data.Add(new KeyValuePair<Guid, string>(item.SalutationId, text));
                }
