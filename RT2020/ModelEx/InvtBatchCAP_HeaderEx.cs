...
{
    /// <summary>
    /// This extension provides access to the EF6 entity dbo.InvtBatchCAP_Header.
    /// Date Created:   2020-12-18 01:31:22
    /// Created By:     Generated by CodeSmith version 7.0.0.15123
    /// Template:       BusinessObjects_v2020.12.12.cst
    /// </summary>
    public class InvtBatchCAP_HeaderEx
    {
        public static bool DeleteChildToo(Guid headerId)
        {
            ...
            {
                var oHeader = ctx.InvtBatchCAP_Header.Find(headerId);
                if (oHeader != null)
                {
                    var details = ctx.InvtBatchCAP_Details.Where(x => x.HeaderId == headerId);
                    foreach (var detail in details)
                    {
                        ctx.InvtBatchCAP_Details.Remove(detail);
                    }

                    ctx.InvtBatchCAP_Header.Remove(oHeader);
                    ctx.SaveChanges();
        ...
        /// <summary>
        /// Get a list of InvtBatchCAP_Header objects from the database
        /// </summary>
        /// <returns>A list containing all of the InvtBatchCAP_Header objects in the database.</returns>
        public static List<EF6.InvtBatchCAP_Header> GetList()
        {
            var whereClause = "1 = 1";
        ...
        /// </summary>
        /// <returns>A list containing all of the InvtBatchCAP_Header objects in the database ordered by the columns specified.</returns>
        public static List<EF6.InvtBatchCAP_Header> GetList(string whereClause)
        {
        ...
        /// </summary>
        /// <returns>A list containing all of the InvtBatchCAP_Header objects in the database.</returns>
        public static List<EF6.InvtBatchCAP_Header> GetList(string whereClause, string[] orderBy)
        {
            List<EF6.InvtBatchCAP_Header> result = new List<EF6.InvtBatchCAP_Header>();
            ...
            {
                result = ctx.InvtBatchCAP_Header
                    .SqlQuery(string.Format("Select * from InvtBatchCAP_Header Where {0} Order By {1}", whereClause, orderby))
                    .AsNoTracking()
                    .ToList();
            }
        ...
        /// <summary>
        /// Deletes a EF6.InvtBatchCAP_Header object from the database.
        /// </summary>
        /// <param name="headerId">The primary key value</param>
        public static bool Delete(Guid headerId)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var item = ctx.InvtBatchCAP_Header.Find(headerId);
                if (item != null)
                {
                    ctx.InvtBatchCAP_Header.Remove(item);
                    ctx.SaveChanges();
                    result = true;
                }
            }
		...
		public static void LoadCombo(ref ComboBox ddList, string TextField, bool SwitchLocale, bool BlankLine, string BlankLineText, string ParentFilter, string WhereClause, string[] OrderBy)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var list = ctx.InvtBatchCAP_Header.SqlQuery(
                    String.Format(
                        "Select * from InvtBatchCAP_Header Where {0} Order By {1}",
                ...
                if (BlankLine)
                {
                    list.Insert(0, new EF6.InvtBatchCAP_Header()
                    {
                        HeaderId = Guid.Empty,
                        TxType = "",
