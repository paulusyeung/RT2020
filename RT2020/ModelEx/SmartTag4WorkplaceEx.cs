...
{
    public class SmartTag4WorkplaceEx
    {
        public static bool IsTagCodeInUse(string code)
        {
            ...
            {
                var tag = ctx.SmartTag4Workplace.Where(x => x.TagCode == code).FirstOrDefault();
        ...
        public static bool DeleteOptionsToo(Guid tagId)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                using (var scope = ctx.Database.BeginTransaction())
                {
                    try
                    {
                        var item = ctx.SmartTag4Workplace.Find(tagId);
                        if (item != null)
                        {
                            #region remove Options
                            var options = ctx.SmartTag4Workplace_Options.Where(x => x.TagId == item.TagId);
                            if (options.Count() > 0)
                            {
                                foreach (var option in options)
                                {
                                    ctx.SmartTag4Workplace_Options.Remove(option);
                                }
                            }
                            #endregion

                            #region remove in use by parents
                            var used = ctx.WorkplaceSmartTag.Where(x => x.TagId == item.TagId);
                            if (used.Count() > 0)
                            {
                                foreach (var use in used)
                                {
                                    ctx.WorkplaceSmartTag.Remove(use);
                                }
                            }
                            #endregion

                            ctx.SmartTag4Workplace.Remove(item);
                            ctx.SaveChanges();
        ...
        public static Guid GetIdByPriority(int priority)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var item = ctx.SmartTag4Workplace.Where(x => x.Priority == priority).FirstOrDefault();
        ...
        public static Guid GetIdByTagCodeAndPriority(string code, int priority)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var item = ctx.SmartTag4Workplace.Where(x => x.TagCode == code && x.Priority == priority).FirstOrDefault();
                if (item != null) result = item.TagId;
            }
        ...
        public static List<EF6.SmartTag4Workplace> GetListOrderBy(string[] orderBy, bool switchLocale = false)
        {
            List<EF6.SmartTag4Workplace> result = new List<EF6.SmartTag4Workplace>();
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                ...
                result = ctx.SmartTag4Workplace.SqlQuery(
                    String.Format(
                        "Select * from SmartTag4Workplace Order By {0}",
                        order
                    ))
                    .AsNoTracking()
                    .ToList();
        ...
        /// <summary>
        /// Get a list of SmartTag4Workplace objects from the database
        /// </summary>
        /// <returns>A list containing all of the SmartTag4Workplace objects in the database.</returns>
        public static List<EF6.SmartTag4Workplace> GetList()
        {
            var whereClause = "1 = 1";
        ...
        /// </summary>
        /// <returns>A list containing all of the SmartTag4Workplace objects in the database ordered by the columns specified.</returns>
        public static List<EF6.SmartTag4Workplace> GetList(string whereClause)
        {
        ...
        /// </summary>
        /// <returns>A list containing all of the SmartTag4Workplace objects in the database.</returns>
        public static List<EF6.SmartTag4Workplace> GetList(string whereClause, string[] orderBy)
        {
            List<EF6.SmartTag4Workplace> result = new List<EF6.SmartTag4Workplace>();
            ...
            {
                result = ctx.SmartTag4Workplace
                    .SqlQuery(string.Format("Select * from SmartTag4Workplace Where {0} Order By {1}", whereClause, orderby))
                    .AsNoTracking()
                    .ToList();
            }
        ...
        /// <summary>
        /// Deletes a EF6.SmartTag4Workplace object from the database.
        /// </summary>
        /// <param name="tagId">The primary key value</param>
		...
		public static void LoadCombo(ref ComboBox ddList, string TextField, bool SwitchLocale, bool BlankLine, string BlankLineText, string ParentFilter, string WhereClause, string[] OrderBy)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                ...
                var kvpList = ctx.Database.SqlQuery<ComboBoxHelper.ComboBoxItem>(
                    string.Format(
                        "Select TagId as [Key], {0} as [Value] from SmartTag4Workplace Where {1} Order By {2}",
		...
		public static void LoadCombo(ref ComboBox ddList, string[] TextField, string TextFormatString, bool SwitchLocale, bool BlankLine, string BlankLineText, string WhereClause, string[] OrderBy)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var list = ctx.SmartTag4Workplace.SqlQuery(
                    String.Format(
                        "Select * from SmartTag4Workplace Where {0} Order By {1}",
