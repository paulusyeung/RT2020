...
{
    /// <summary>
    /// This extension provides access to the EF6 entity dbo.InvtBatchTXF_Header.
    /// Date Created:   2020-12-17 07:34:45
    ...
    public class InvtBatchTXF_HeaderEx
    {
        public static bool DeleteChildToo(Guid headerId)
        {
            ...
            {
                var oHeader = ctx.InvtBatchTXF_Header.Find(headerId);
                if (oHeader != null)
                {
                    var details = ctx.InvtBatchTXF_Details.Where(x => x.HeaderId == headerId);
                    foreach (var detail in details)
                    {
                        ctx.InvtBatchTXF_Details.Remove(detail);
                    }
        ...
        public static bool IsTxNumberInUse(string txNumber)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var oHeader = ctx.InvtBatchTXF_Header.Where(x => x.TxNumber == txNumber || x.TxNumber.Contains(txNumber)).AsNoTracking().FirstOrDefault();
                if (oHeader != null) result = true;
            }
        ...
        /// <summary>
        /// Get a list of InvtBatchTXF_Header objects from the database
        /// </summary>
        /// <returns>A list containing all of the InvtBatchTXF_Header objects in the database.</returns>
        public static List<EF6.InvtBatchTXF_Header> GetList()
        {
            var whereClause = "1 = 1";
        ...
        /// </summary>
        /// <returns>A list containing all of the InvtBatchTXF_Header objects in the database ordered by the columns specified.</returns>
        public static List<EF6.InvtBatchTXF_Header> GetList(string whereClause)
        {
        ...
        /// </summary>
        /// <returns>A list containing all of the InvtBatchTXF_Header objects in the database.</returns>
        public static List<EF6.InvtBatchTXF_Header> GetList(string whereClause, string[] orderBy)
        {
            List<EF6.InvtBatchTXF_Header> result = new List<EF6.InvtBatchTXF_Header>();
            ...
            {
                result = ctx.InvtBatchTXF_Header
                    .SqlQuery(string.Format("Select * from InvtBatchTXF_Header Where {0} Order By {1}", whereClause, orderby))
                    .AsNoTracking()
                    .ToList();
            }
        ...
        /// <summary>
        /// Deletes a EF6.InvtBatchTXF_Header object from the database.
        /// </summary>
        /// <param name="headerId">The primary key value</param>
        public static bool Delete(Guid headerId)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var item = ctx.InvtBatchTXF_Header.Find(headerId);
                if (item != null)
                {
                    ctx.InvtBatchTXF_Header.Remove(item);
                    ctx.SaveChanges();
                    result = true;
                }
            }
		...
		public static void LoadCombo(ref ComboBox ddList, string TextField, bool SwitchLocale, bool BlankLine, string BlankLineText, string ParentFilter, string WhereClause, string[] OrderBy)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var list = ctx.InvtBatchTXF_Header.SqlQuery(
                    String.Format(
                        "Select * from InvtBatchTXF_Header Where {0} Order By {1}",
                ...
                if (BlankLine)
                {
                    list.Insert(0, new EF6.InvtBatchTXF_Header()
                    {
                        HeaderId = Guid.Empty,
                        TxType = "",
