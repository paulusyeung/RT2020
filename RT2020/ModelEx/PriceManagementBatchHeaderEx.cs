...
{
    public class PriceManagementBatchHeaderEx
    {
        public static bool IsTxNumberInUse(string txNumber)
        {
            ...
            {
                var item = ctx.PriceManagementBatchHeader.Where(x => x.TxNumber == txNumber).FirstOrDefault();
        ...
        //{
            ...
            {
                var list = ctx.PriceManagementBatchHeader.SqlQuery(
                    String.Format(
                        "Select * from PriceManagementBatchHeader Where {0} Order By {1}",
                ...
                if (BlankLine)
                {
                    list.Insert(0, new EF6.PriceManagementBatchHeader()
                    {
                        HeaderId = Guid.Empty,
                        TxNumber = "",
