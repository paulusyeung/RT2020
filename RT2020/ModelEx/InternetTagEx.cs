...
{
    public class InternetTagEx
    {
        public static bool IsInternetTagCodeInUse(string code)
        {
            bool result = false;

            using (var ctx = new EF6.RT2020Entities())
            {
                var tag = ctx.InternetTag.Where(x => x.TagCode == code).FirstOrDefault();
        ...
        public static void LoadCombo(ref ComboBox ddList, string[] TextField, string TextFormatString, bool SwitchLocale, bool BlankLine, string BlankLineText, string ParentFilter, string WhereClause, string[] OrderBy)
        {
            ...
            {
                var list = ctx.InternetTag.SqlQuery(
                    String.Format(
                        "Select * from InternetTag Where {0} Order By {1}",
                ...
                ddList.DisplayMember = !SwitchLocale ? "TagName" :
