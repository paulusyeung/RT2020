    ...
    public class WorkplaceZoneEx
    {
        ...
        public static void LoadCombo(ref ComboBox ddList, string[] TextField, string TextFormatString, bool SwitchLocale, bool BlankLine, string BlankLineText, string ParentFilter, string WhereClause, string[] OrderBy)
        {
            ...
            using (var ctx = new RT2020.EF6.RT2020Entities())
            {
                ...
                if (BlankLine)
                {
                    list.Insert(0, new EF6.WorkplaceZone()
                    {
                        ZoneId = Guid.Empty,
                        ZoneCode = "",
                        ZoneName = BlankLineText,
                        ZoneName_Chs = BlankLineText,
                        ZoneName_Cht = BlankLineText,
                    });
                }
                #endregion

                ddList.DataSource = list;
                ddList.ValueMember = "ZoneId";
                ddList.DisplayMember = !SwitchLocale ? "ZoneName" :
