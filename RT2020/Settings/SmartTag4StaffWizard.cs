...
namespace RT2020.Settings
{
    public partial class SmartTag4StaffWizard : Form
    {
        ...
        private bool CodeExists()
        {
            ...
            SmartTag4StaffCollection tagList = SmartTag4Staff.LoadCollection(sql);
            if (tagList.Count > 0)
            {
                return true;
            }
        ...
        private bool Save()
        {
            ...
            else
            {
                ...
                SmartTag4Staff oTag = SmartTag4Staff.Load(this.TagId);
                if (oTag == null)
                {
                    oTag = new SmartTag4Staff();

                    if (CodeExists())
                    {
                        errorProvider.SetError(txtTagCode, string.Format(Resources.Common.DuplicatedCode, "SmartTag Code for Staff"));
                        return false;
                    }
                    else
                    {
        ...
        private void lvTagList_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lvTagList.SelectedItem != null)
            {
                ...
                {
                    SmartTag4Staff oTag = SmartTag4Staff.Load(new System.Guid(lvTagList.SelectedItem.Text));
                    if (oTag != null)
                    {
