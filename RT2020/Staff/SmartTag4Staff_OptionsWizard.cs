...
{
    public partial class SmartTag4Staff_OptionsWizard : Form
    {
        ...
        public SmartTag4Staff_OptionsWizard()
        {
            InitializeComponent();
        }
        ...
        private void BindTagList()
        {
            ...
            {
                var list = ctx.SmartTag4Staff_Options.Where(x => x.TagId == _SmartTagId).OrderBy(x => x.OptionCode).AsNoTracking().ToList();
        ...
        private bool IsValid()
        {
            ...
            errorProvider.SetError(txtOptionCode, string.Empty);
            if (_OptionId == Guid.Empty && ModelEx.SmartTag4Staff_OptionsEx.IsTagCodeInUse(_SmartTagId, txtOptionCode.Text.Trim()))
            {
        ...
        private void lvTagList_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lvTagList.SelectedItem != null)
            {
                ...
                if (Guid.TryParse(lvTagList.SelectedItem.Text, out id))
                {
                    ...
                    using (var ctx = new EF6.RT2020Entities())
                    {
                        var item = ctx.SmartTag4Staff_Options.Find(id);
                        if (item != null)
                        {
