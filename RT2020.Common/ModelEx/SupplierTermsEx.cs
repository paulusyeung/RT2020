...
namespace RT2020.Common.ModelEx
{
    public class SupplierTermsEx
    {
        ...
        public static string GetTermsCode(Guid id)
        {
            string result = null;
            ...
            {
                var st = ctx.SupplierTerms.Find(id);
                if (st != null) result = st.TermsCode;
            }

            return result;
        }

        public static bool IsTermsCodeInUse(string code)
        {
            bool result = false;

            using (var ctx = new EF6.RT2020Entities())
            {
                var st = ctx.SupplierTerms.Where(x => x.TermsCode == code).FirstOrDefault();
                if (st != null) result = true;
            }
        ...
        //{
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var list = ctx.SupplierTerms.SqlQuery(
                    String.Format(
                        "Select * from SupplierTerms Where {0} Order By {1}",
                ...
                if (BlankLine)
                {
                    list.Insert(0, new EF6.SupplierTerms()
                    {
                        TermsId = Guid.Empty,
                        TermsCode = "",
                        TermsName = BlankLineText,
                        TermsName_Chs = BlankLineText,
                        TermsName_Cht = BlankLineText,
                    });
                }
                #endregion

                ddList.DataSource = list;
                ddList.ValueMember = "TermsId";
                ddList.DisplayMember = !SwitchLocale ? TextField :
                    CookieHelper.CurrentLocaleId == LanguageHelper.AlternateLanguage2.Key ?
                    "TermsName_Cht" : CookieHelper.CurrentLocaleId == LanguageHelper.AlternateLanguage1.Key ?
                    "TermsName_Chs" :
                    "TermsName";
            }
