...
{
    /// <summary>
    /// This extension provides access to the EF6 entity dbo.EPOSBatchHeader.
    /// Date Created:   2020-12-18 07:18:51
    /// Created By:     Generated by CodeSmith version 7.0.0.15123
    /// Template:       BusinessObjects_v2020.12.12.cst
    /// </summary>
    public class EPOSBatchHeaderEx
    {
        public static bool DeleteChildToo(Guid headerId)
        {
            ...
            {
                var oHeader = ctx.EPOSBatchHeader.Find(headerId);
                if (oHeader != null)
                {
                    var details = ctx.EPOSBatchDetails.Where(x => x.HeaderId == headerId);
                    foreach (var detail in details)
                    {
                        ctx.EPOSBatchDetails.Remove(detail);
                    }
                    var tenders = ctx.EPOSBatchTender.Where(x => x.HeaderId == headerId);
                    foreach (var tender in tenders)
                    {
                        ctx.EPOSBatchTender.Remove(tender);
                    }

                    ctx.EPOSBatchHeader.Remove(oHeader);
                    ctx.SaveChanges();
        ...
        /// <summary>
        /// Get a list of EPOSBatchHeader objects from the database
        /// </summary>
        /// <returns>A list containing all of the EPOSBatchHeader objects in the database.</returns>
        public static List<EF6.EPOSBatchHeader> GetList()
        {
            var whereClause = "1 = 1";
        ...
        /// </summary>
        /// <returns>A list containing all of the EPOSBatchHeader objects in the database ordered by the columns specified.</returns>
        public static List<EF6.EPOSBatchHeader> GetList(string whereClause)
        {
        ...
        /// </summary>
        /// <returns>A list containing all of the EPOSBatchHeader objects in the database.</returns>
        public static List<EF6.EPOSBatchHeader> GetList(string whereClause, string[] orderBy)
        {
            List<EF6.EPOSBatchHeader> result = new List<EF6.EPOSBatchHeader>();
            ...
            {
                result = ctx.EPOSBatchHeader
                    .SqlQuery(string.Format("Select * from EPOSBatchHeader Where {0} Order By {1}", whereClause, orderby))
                    .AsNoTracking()
                    .ToList();
            }
        ...
        /// <summary>
        /// Deletes a EF6.EPOSBatchHeader object from the database.
        /// </summary>
        /// <param name="headerId">The primary key value</param>
        public static bool Delete(Guid headerId)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var item = ctx.EPOSBatchHeader.Find(headerId);
                if (item != null)
                {
                    ctx.EPOSBatchHeader.Remove(item);
                    ctx.SaveChanges();
                    result = true;
                }
            }
		...
		public static void LoadCombo(ref ComboBox ddList, string TextField, bool SwitchLocale, bool BlankLine, string BlankLineText, string ParentFilter, string WhereClause, string[] OrderBy)
        {
            ...
            using (var ctx = new EF6.RT2020Entities())
            {
                var list = ctx.EPOSBatchHeader.SqlQuery(
                    String.Format(
                        "Select * from EPOSBatchHeader Where {0} Order By {1}",
                ...
                if (BlankLine)
                {
                    list.Insert(0, new EF6.EPOSBatchHeader()
                    {
                        HeaderId = Guid.Empty,
                        TxType = "",
