    ...
    public partial class HistoryDetail : Form
    {
        ...
        private string BuildQuery()
        {
            ...
            switch (this.txtType.Text.Trim())
            {
                case "SHOP":
                    query = @"
SELECT     fd.DetailsId, fd.HeaderId, fd.LineNumber, fd.ProductId, p.STKCODE, p.APPENDIX1, p.APPENDIX2, p.APPENDIX3, fd.CONFIRM_TRF_QTY
...
WHERE       fd.HeaderId = '" + this.HeaderId.ToString() + "' ORDER BY fd.LineNumber";
                ...
                case "WH":
                    query = @"
SELECT     td.DetailsId, td.HeaderId, td.LineNumber, td.ProductId, p.STKCODE, p.APPENDIX1, p.APPENDIX2, p.APPENDIX3, td.QtyConfirmed
